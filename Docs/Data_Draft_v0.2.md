# Little Paw Town - Data Draft v0.2

---

## 1. 문서 목적

본 문서는 `Little Paw Town GDD v0.2` 기준으로, MVP 구현에 필요한 **핵심 데이터 구조 초안**을 정리한 문서다.
...
| bgm_key | string | 배경음 |
| preview_text_key | string | 장소 카드/상태 패널 미리보기 텍스트 키 |
| ambient_reaction_tags | string[] | 장소 내 짧은 관찰 연출 태그 |
...
> 홈 진입 직후 짧게 재생되는 생활 반응은 `presentation_type=ambient`, 본격 장면 이벤트는 `presentation_type=scene`으로 구분해 운용하는 편이 좋다.
...
| presentation_type | enum | ambient, scene, push |
| entry_point | enum | home_auto, home_suggested, location_direct, push |
| entry_cta_text_key | string | 홈 상태 패널 추천 CTA 텍스트 키 |
...
| current_state | enum | idle, resting, exploring, watching, sniffing 등 |
...
| 생성 완료 여부 | 생성 이탈 확인 |
| 장소 이동 기록 | 루프 사용성 확인 |
| 홈 생활 반응 노출/탭 | 홈 관찰 재미 검증 |
| 상태 패널 CTA 클릭 | 홈에서 장소/행동 유도 효율 검증 |
