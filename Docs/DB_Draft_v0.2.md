# Little Paw Town - DB Draft v0.2

---

## 1. 문서 목적

본 문서는 `Little Paw Town GDD v0.2` 및 `Data Draft v0.2`을 기준으로, MVP 구현에 필요한 **관계형 DB 스키마 초안**을 정리한 문서다.
...
| preview_text_key | VARCHAR(100) | NULL | 장소 카드/상태 패널 미리보기 텍스트 키 |
| ambient_reaction_tags_json | JSON | NULL | 장소 내 짧은 관찰 연출 태그 |
...
| presentation_type | VARCHAR(20) | NOT NULL DEFAULT 'scene' | `ambient`, `scene`, `push` |
| entry_point | VARCHAR(30) | NOT NULL DEFAULT 'location_direct' | `home_auto`, `home_suggested`, `location_direct`, `push` |
| entry_cta_text_key | VARCHAR(100) | NULL | 홈 상태 패널 CTA 텍스트 키 |
...
**Index**
- `idx_event_template_master_enabled_category (enabled, category, presentation_type)`
...
> 홈 진입 직후 생활 반응은 `presentation_type=ambient`, 장소에서 소비하는 본편 이벤트는 `presentation_type=scene`으로 구분해 조회하는 편이 적절하다.
...
| current_state | VARCHAR(30) | NOT NULL DEFAULT 'idle' | 현재 상태 (`idle`, `resting`, `exploring`, `watching`, `sniffing` 등) |
...
## 10.5 홈 허브 이벤트 계층
홈 화면은 관찰 허브이므로, `event_template_master.presentation_type`과 `entry_point`를 사용해 **생활 반응 / 추천 진입 / 본편 이벤트**를 구분하는 편이 적절하다.  
이렇게 하면 홈 강제 팝업을 줄이면서도 추천 CTA와 장소 이벤트를 동일 이벤트 시스템 안에서 관리할 수 있다.
